...
// void showExerciseScreen(int index){
...
// 				return true;
// 			}
// 			for (int i = 0; i < token.exercises[currentExerciseIndex].expected.size() || i < results.size(); ++i){
...
// 	auto container = Container::Vertical({input, prev_button, next_button, back_button});

// 	auto main_component = Renderer(container, [&] {
// 		std::vector<Element> result_elements;
// 		for (const std::string& result : results) {
// 			result_elements.push_back(text(result) | color(Color::Yellow) | bold);
// 		}

//         token.completed = token.exercises[token.exercises.size() - 1].completed;

// 		return vbox({
// 			text("") | size(HEIGHT, EQUAL, 2),
// 			text("Token " + token.token) | bold | underlinedDouble | center,
// 			text("") | size(HEIGHT, EQUAL, 1),
// 			window(text(" Description "),
// 				paragraph(token.description)
// 			),
// 			separator(),
// 			window(text(" Exercise " + std::to_string(currentExerciseIndex + 1) + " "),
// 				vbox({
// 					hbox(text("Instruction:") | bold | color(Color::Cyan), filler()),
// 					paragraph(token.exercises[currentExerciseIndex].description) | color(Color::White),
// 					separator(),
// 					hbox(text("Text:") | bold | color(Color::Cyan), filler()),
// 					paragraph(token.exercises[currentExerciseIndex].text) | color(Color::Green),
// 				})
// 			),
// 			container->Render() | border | size(HEIGHT, EQUAL, 3),
// 			vbox(result_elements),
// 			filler(),
// 			hbox({
// 				(currentExerciseIndex > 0 ? prev_button ->Render() : filler()),
// 				filler(),
// 				(currentExerciseIndex + 1 < token.exercises.size() && token.exercises[currentExerciseIndex].completed ? next_button->Render() : filler()),
// 			}),
...
//             showExerciseScreen(tokens[selected_index], &should_return_to_menu);

//             if (!should_return_to_menu) {
//                 break; // Quitte l'application si l'utilisateur ne veut pas revenir
//             }
//         } else {
//             break; // Quitte si on a quitté le menu sans lancer d'exercice
//         }
//     }
// 	return 0;
// }
    ...
    while (currentTokenIndex < tokens.size()) {
        ...
        auto main_component = Renderer(container, [&] {
            ...
            return vbox({
                ...
                separator(),
                window(text(" Description "), paragraph(token.description)),
                separator(),
                window(text(" Exercise " + std::to_string(currentExerciseIndex + 1)),
                    vbox({
